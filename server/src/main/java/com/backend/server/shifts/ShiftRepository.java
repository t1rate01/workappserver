...
public interface ShiftRepository extends JpaRepository<Shift, Long>{
    ...
    @Query(value = "SELECT * FROM shifts WHERE user_id = :userId AND date = :date ORDER BY date DESC LIMIT 1", nativeQuery = true)
    Optional<Shift> findByUserAndDate(@Param("userId") Long userId, @Param("date") LocalDate date);

    // kaikki käyttäjän tänään ja tulevat vuorot
    @Query(value = "SELECT * FROM shifts WHERE user_id = :userId AND date >= :date ORDER BY date ASC", nativeQuery = true)
    List<Shift> findFutureShiftsByUserId(@Param("userId") Long userId, @Param("date") LocalDate date);
